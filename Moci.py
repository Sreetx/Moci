...
while True:
    try:
        ...
        print("  {6} Brute Force Website")
        print("  {7} Israel SQL Injection Dork         [#] Konten luar ")
        print("  {8} Server Info                        {S} Sqlmap")
        print("  {9} Readme/Tentang                     {M} Metasploit (Khusus Linux)")
        print('  {10} Keluar')
        pilih = input("\n   [?] Pilih opsi: ")
    ...
    # Tool's kami
        if pilih.strip() == '1':
            ...
            if tanya13.lower() == 'y':
                print('\n [*] Mengunduh script....')
                df = requests.get('https://github.com/Sreetx/defacer/archive/reft/heads/main.zip')
                nf4 = df.split('/')[-1]
            ...
            print('\n [*] Deskripsi:')
            print('  Brute force website adalah sebuah script yang digunakan untuk menyerang website dengan cara DDOS Attack dan juga Deface atau mengirim virus secara bersamaan. Kami tidak bertanggung jawab atas apapun yang kalian lakukan, apapun itu.')
            tanya9 = input('\n [?] Pasang script? [Y/n]: ')
            if tanya9.lower() == 'y':
                print('\n [*] Mengunduh script....')
                brtf = requests.get('https://github.com/Sreetx/brute-force/archive/refs/heads/main.zip')
                nf6 = brtf.split('/')[-1]
                with open(nf6, 'wb') as code5:
                  code5.write(brtf.content)
                print(' [*] Selesai')
                print(" [*] File disimpan dengan nama '"+nf6+"'\n")
        elif pilih.strip() == '7':
            print('\n [*] Deskripsi:')
